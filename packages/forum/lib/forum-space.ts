/**
 * 论坛一级分区：大学论坛、兴趣圈子、本地同城。
 * 大学只是其中一类；圈子/同城登录即可发帖，不走高校实名。
 */
...
export const FORUM_SPACE_KINDS = ["UNIVERSITY", "CIRCLE", "CITY"] as const;
...
export async function ensureStarterForumSpaces(kind: "CIRCLE" | "CITY") {
  const count = await prisma.forumUniversity.count({ where: { kind } });
  if (count > 0) return;
  const starters = kind === "CIRCLE" ? STARTER_CIRCLES : STARTER_CITIES;

/**
 * 大学论坛业务规则：高校分区、专区、发帖权限、互动动作。
 * 账号与全站同一套 User / 登录 Cookie，不另开论坛号；微信、登录名、手机、邮箱均可。
 * 发帖仅本校成员（或站长）；浏览对所有人开放。
...
export function canPostInUniversity(
  session: { forumUniversityId?: string | null } & RoleInput,
  universityId: string,
): boolean {
  if (isAdmin(session)) return true;
  return Boolean(session.forumUniversityId && session.forumUniversityId === universityId);
}

...
import { getOrderFormConfig, getUiCopy } from "@/lib/site-settings";
...
export default async function StudioCmsPage() {
  ...
  const [uiCopy, orderForm] = await Promise.all([
    getUiCopy(),
    getOrderFormConfig(),
  ]);
    ...
    <div className="container space-y-6 py-12">
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">
          管理后台文案与下单信息采集字段。视觉门面请到「店铺装修」。
        </p>
      </div>
      <CmsPanel initial={{ uiCopy, orderForm }} />
    </div>
  );
}

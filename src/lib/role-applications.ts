 ...
 * - 个人中心升级：保留当前 role，仅写 requestedRole + PENDING；不可申请 ADMIN；
 *   不可申请已有角色；已有 PENDING 时不可再提
 * - 通过：role=requestedRole，status=ACTIVE
 * - 拒绝：不改当前 role（注册路径本就是 STUDENT；升级路径保留原身份），status=REJECTED
 * - 站长直接改角色：同步清掉待审状态（见 fieldsAfterManualRoleChange）
 *
 * 权限矩阵仍以 src/lib/roles.ts 为准；本模块只处理「申请字段怎么写」。
 */
...
/**
 * 个人中心可申请的目标角色（相对当前身份的「升级/入驻」路径）。
 * 不可自选站长；已是该角色则不出现在列表。
 */
const ACCOUNT_APPLY_TARGETS: Record<Role, ElevatedApplyRole[]> = {
  ADMIN: [],
  AGENT: [],
  MERCHANT: ["AGENT"],
  TEACHER: ["MERCHANT", "AGENT"],
  STUDENT: ["AGENT", "MERCHANT", "TEACHER"],
};
...
/**
 * 已登录用户在个人中心提交角色申请。
 * 不改当前 role，避免升级待审期间丢掉老师/商家等既有权限。
 */
...
/**
 * 站长在用户列表直接改角色时，顺带收口申请状态，避免残留「待审核」。
 */
export function fieldsAfterManualRoleChange(role: Role, reviewerId: string) {
  return {
    role,
    requestedRole: role === "STUDENT" || role === "ADMIN" ? "" : role,
    roleApplicationStatus:
      role === "STUDENT" ? ("NONE" as const) : ("ACTIVE" as const),
    roleApplicationNote: "",
    roleReviewedAt: new Date(),
    roleReviewedById: reviewerId,
  };
...
export function availableAccountApplyRoles(
  ...
  const targets = ACCOUNT_APPLY_TARGETS[currentRole] || [];
  return targets.filter((role) => role !== currentRole);
}
...
export function validateAccountRoleApply(
  ...
  const allowed = availableAccountApplyRoles(currentRole, applicationStatus);
  if (!allowed.includes(requestedRole)) {
    return {
      ok: false,
      error: `当前身份（${ROLE_LABEL[currentRole]}）不可申请${ROLE_LABEL[requestedRole]}`,
    };
  }
...
export function approveSuccessMessage(role: string): string {
  return `已通过，角色设为${ROLE_LABEL[role as Role] || role}`;
}
